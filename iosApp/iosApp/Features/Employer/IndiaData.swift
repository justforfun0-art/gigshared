...
/// Static Indian states/districts + employer dropdown options — port of Android's
/// `IndiaData` (the subset the employer profile-edit screen uses). Kept on the
/// iOS side (not in the shared KMP core) since it's UI-picker data, matching how
/// the Android app keeps it in its UI layer.
enum IndiaData {
    ...
    /// Districts per state (mirrors Android `districtsByState` / web indian-states.ts).
    static let districtsByState: [String: [String]] = [
    ...
    /// Industries shown in the employer profile-edit dropdown (Android edit screen's list).
    static let industries = [
        "Agriculture", "Construction", "Education", "Food & Beverage", "Healthcare",
        "Hospitality", "IT & Software", "Manufacturing", "Retail", "Transportation",
        "Other",
    ]

    static let companySizes = [
